...
if(!kode) return msg.channel.send('There are no parameters to execute. 🤦‍♂️');
exec(kode, async (err, sonuç, hata) => {
    
if(sonuç){
if(sonuç.length > 2047) {
hastebin(sonuç, { extension: "js" }).then(cıkra => { msg.channel.send('An error has occurred I cannot download embedi'+cıkra+''); })
return
}
...
if(hata.length > 2047) {
 hastebin(hata, { extension: "js" }).then(cıkra => { msg.channel.send('An error has occurred I cannot download embedi '+cıkra+''); })
return
 }
